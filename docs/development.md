...
| Provider Protocol + Claude/OpenAI adapters | `embed` and `answer` are injected into services; HTTP wire formats are confined to `ai/provider.py`, `ai/investigator.py`, and `ai/anthropic.py`. Reasoning selection is independent of OpenAI embeddings. Adding a vendor also requires extending configuration and the index fingerprint/status wiring. No SDK dependency was necessary because httpx was already installed. | Which parts of a provider swap invalidate stored vectors? |
...
The reasoning adapters use provider-specific structured JSON outputs for a decision envelope containing a public plan, action, arguments, and optional final claims. Server Pydantic validation and a static dispatch table remain authoritative. Native hosted shell/tools are not enabled. Other providers can implement `InvestigationProvider.decide` and `embed`; existing Q&A behavior remains independent.
