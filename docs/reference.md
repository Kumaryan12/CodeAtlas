# CodeAtlas technical reference

Configuration, API routes, runtime boundaries, and detailed usage. For the project overview and measured results, return to the [README](../README.md).
...
Source is stored once per file. Symbols store names, types, parameters, parent IDs, and line ranges; source snippets can be derived from the file without duplicating class/function bodies. Structured imports and captured alias configuration feed a deterministic, on-demand file graph; dependency-graph reads do not load source text or invoke parsers. The separate Data Flow view reads stored Python source for bounded value-flow analysis. Dependency edges describe static imports, not runtime execution.

See [development guide](development.md) for architecture trade-offs and interview questions, [source tree](structure.md), and [verification record](verification.md).
...
The [24-case RAG benchmark](rag-evaluation.md) and [measured results](rag-results-review.md) add real local embeddings, four retrieval ablations, unsupported/adversarial questions, and cited-answer evaluation through production indexing and Q&A. Saved reports distinguish service failures, structural citation checks, and manual factual review.
...
# With configured local or remote embeddings: compare three retrieval modes.
...
[Recorded offline results](evaluation-m4-offline.json): lexical Recall@6 **0.9167**, MRR@6 **0.8125**; graph expansion left these aggregate results unchanged. This is the historical 12-question baseline. The newer [RAG evaluation](rag-evaluation.md) records real local embeddings and live answers on a separate corpus; the datasets are not directly comparable. Numerical answer faithfulness and larger held-out evaluations remain outstanding. See [verification notes](verification.md).
...
The [MCP integration](mcp-architecture.md) connects the agent runtime to a bundled, repository-scoped stdio server for file listing, source reads, symbol lookup, and dependency inspection. Enable `CODEATLAS_READ_TOOL_TRANSPORT=mcp` and restart the API; the trace labels these reads **READ · MCP**. Search, draft edits, and approved Docker tests retain their local handlers. The six MCP tests include a real subprocess agent workflow and require no model key. The first [live Claude/MCP evaluation](evaluation-agent-live.md) completed all six runs: four automatic passes and two strict marker-check failures that require interpretation. Broader agent quality remains unmeasured; the [repair readiness follow-up](resume-readiness-evidence.md) adds repeated synthetic edit-and-test runs.
...
Run the six-case scripted evaluation without a key, or opt into live evaluation after configuration. Both local and MCP transports are supported. See [evaluation setup and limitations](agent-evaluation.md), the [scripted report](evaluation-agent-scripted.md), and its [JSON detail](evaluation-agent-scripted.json). CI produces fresh scripted reports. Passing scripted cases establishes runtime behavior, not live model quality or claim faithfulness.

The first [live evaluation report](evaluation-agent-live.md) preserves all failures and manual review notes; [its JSON](evaluation-agent-live.json) includes measured usage and complete traces. It is a small integration baseline, not a reliability benchmark.
